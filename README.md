...
*Fig 2: Graph Model Above Populated With Real World Data*
...
- `MERGE` and `ON CREATE` prevents duplication of data when re-loading data into Neo4J, or re-creating relationships. It's good to refactor all node and relationship creation, to have the `MERGE` clause, and set up properties or aggregations with `ON CREATE`. Together, both reads as: 'Match this node/relationship (via `MERGE`), and if there is an existing match, ignore. If there is no match, create. Then on creation (via `ON CREATE`), set stuff up. `ON CREATE` ensures that properties are only set up upon first instantiation of nodes/relationships.
